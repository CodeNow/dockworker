...
var deathTimer = setTimeout(server.close.bind(server, doom), 1000 * 30);

server.on('connection', function (conn) {
  server.once('closing', conn.end.bind(conn));
});
...
function doom () {
  ...
  if (server._handle) { //bad way to check if server is listening, need to maintain state internally
