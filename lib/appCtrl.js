...
function killed () {
  return theApp.signalCode !== null || 
    theApp.killed !== false ||
    theApp.exitCode !== null;
}
...
function stopApp(cb) {
  ...
  domain.run(function () {
    if (theApp.pid) {
      domain.add(theApp);
      exec('kill ' + theApp.pid, domain.intercept(function (stdout, stderr) {
        cb();
        cb = null;
      }));
    } else {
...
app.get('/api/running', function (req, res) {
  res.json(200, { running: !killed(),
    pid: theApp.pid,
    exitCode: theApp.exitCode,
    signalCode: theApp.signalCode,
    killed: theApp.killed 
  });
});
