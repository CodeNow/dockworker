...
app.get('/', function (req, res) {
  ...
  async.waterfall([
    ...
    function (exists, cb) {
      if (exists) {
        fs.stat(req.query.path, cb);
      } else {
        res.send('400', 'file does not exist');
      }
    }
  ], req.domain.intercept(function (stat) {
    if (stat.isFile()) {
...
app.get('/list', function (req, res) {
  var finder = findit(req.query.path);
  var files = [];
...
function writeStream (req, res) {
  ...
  if (req.query.isDirectory) {
    ...
    req
      .pipe(zlib.createGunzip())
      .pipe(fs.createWriteStream(req.query.path))
      .on('finish', function () {
        res.send(200);
      });
  }
...
app.post('/create', mw.dirty, mw.json, mw.filePath, mw.fileAbsent, function (req, res, next) {
...
app.post('/stream', mw.dirty, function (req, res, next) {
  var form = new formidable.IncomingForm();
  form.parse(req, function (err, fields, files) {
    if (err) {
      next({
        code: 500,
        message: 'error streaming file contents to runnable container'
      });
    } else {
      ...
      writeStream.on('close', function () {
        if (error) {
          next({
            code: 500,
            message: 'error streaming file from temp directory '
          });
        } else {
          res.json(201, {});
        }
...
app.post('/update', mw.dirty, mw.json, mw.filePath, mw.fileExists, function (req, res, next) {
  fs.writeFile(req.filePath, req.body.content, 'utf8', function (err) {
    if (err) {
      if (err.errno === 28) {
        next({
          code: 403,
          message: 'cannot update contents of a directory'
        });
      } else {
        next(err);
      }
...
app.post('/delete', mw.dirty, mw.json, mw.filePath, mw.fileExists, function (req, res, next) {
  fs.unlink(req.filePath, req.domain.intercept(function () {
    res.json(201, {});
  }));
});
app.post('/rename', mw.dirty, mw.json, mw.filePath, mw.newFilePath, mw.fileExists, mw.newFileAbsent, function (req, res, next) {
  fs.rename(req.filePath, req.newFilePath, req.domain.intercept(function () {
    res.json(201, {});
  }));
});
app.post('/move', mw.dirty, mw.json, mw.filePath, mw.newFilePath, mw.fileExists, mw.newFileAbsent, function (req, res, next) {
  fs.rename(req.filePath, req.newFilePath, function (err) {
    if (err) {
      if (err.errno === 18) {
        next({
          code: 403,
          message: 'cannot move path into itself'
        });
      ...
      var read_directory = domain.bind(function (currPath, currVirtualPath, cb) {
          fs.readdir(currPath, domain.bind(function (err, diskFiles) {
            if (err) {
              cb({
                code: 500,
                msg: 'error reading directory, are you root?'
              });
            } else {
              // filer, map
              async.forEach(diskFiles, domain.bind(function (file, cb) {
                        ...
                        if (getContent) {
                          fs.readFile(diskFile, 'utf8', domain.bind(function (err, content) {
                            if (err) {
                              cb({
                                code: 500,
                                msg: 'error reading file from disk'
                              });
                            } else {
                              files.push({
                                name: file,
                ...
                if (err) {
                  cb({
                    code: 500,
                    msg: 'error reading files from container'
                  });
                } else {
                  cb(null, files);
                }
...
app.post('/mkdir', mw.dirty, mw.json, mw.filePath, mw.fileAbsent, function (req, res, next) {
...
app.post('/rmdir', mw.dirty, mw.json, mw.filePath, mw.fileExists, function (req, res, next) {
