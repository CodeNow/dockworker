...
function logHandler (stream) {
  return function (remote) {
    ...
    function follow () {
      ...
      if (theApp.child) {
        if (stream === 'build' && !theApp.buildRunning) {
          console.log('build already finished');
          serverEvents.emit('code', theApp.buildCode); // build already finished
          ts.end();
        }
        else {
          listenToChild();
        ...
        theApp.dispatch.once('attached:child', listenToChild); // child not attached, listen for event
      }
      function listenToChild (processName) {
        console.log('listenToChild ', processName)
        theApp.child.on('close', function (code) {
