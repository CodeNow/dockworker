...
function getFullCommand () {
  var runBuild = buildCommand && build.dirty;
  var command = '';
  if (runBuild) {
    command += 'echo "\033[1;32mExecuting Build Command: \033[0;37m' + buildCommand + '\033[0m\n" && ';
    command += buildCommand + ' && ';
    command += 'curl -X POST http://localhost:15000/api/clean && ';
  }
