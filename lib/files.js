...
app.post('/create', mw.filePath, mw.fileAbsent, function (req, res) {
  // this should be a streaming path
  fs.writeFile(req.filePath, req.body.content, 'utf8', function (err) {
    if (err) {
      if (err.errno === 34) {
        res.json(403, { message: 'path does not exist' }); 
      } else {
        req.domain.emit('error', err);
      }
    } else {
      res.json(201, { });
    }
...
app.post('/read', mw.filePath, mw.fileExists, function (req, res) {
  // this should be a streaming path
...
app.post('/update', mw.filePath, mw.fileExists, function (req, res) {
  // this should be a streaming path
  fs.writeFile(req.filePath, body.content, 'utf8', function (err) {
    if (err) {
      if (err.errno === 28) {
        res.json(403, { message: 'cannot update contents of a directory'});
      } else {
        req.domain.emit('error', err);
      }
    } else {
...
app.post('/delete', mw.filePath, mw.fileExists, function (req, res) {
  fs.unlink(req.filePath, req.domain.intercept(function () {
    res.json(201, { });
  }));
});

app.post('/rename', mw.filePath, mw.newFilePath, mw.fileExists, mw.newFileAbsent, function (req, res) {
  fs.rename(req.filePath, req.newFilePath, req.domain.intercept(function () {
    res.json(201, { });
  }));
});

app.post('/move', mw.newFilePath, mw.fileExists, mw.newFileAbsent, function (req, res) {
  fs.rename(req.filePath, newFilePath, function (err) {
    if (err) {

      if (err.errno === 18) {
        res.json(403, { message: 'cannot move path into itself'});
      } else if (err.errno === 34) {
        res.json(403, { message: 'destination does not exist'});
      } else if (err.errno === 27) {
        res.json(403, { message: 'destination is not a directory'});
      } else {
        req.domain.emit('error', err);
      }
    } else {
...
app.post('/mkdir', mw.filePath, mw.fileAbsent, function (req, res) {
...
app.post('/rmdir', mw.filePath, mw.fileExists, function (req, res) {
  if (req.body.recursive) {
    ...
    fs.rmdir(req.filePath, function (err) {
      if (err) {
        if (err.errno === 53) {
          res.json(403, { message: 'directory is not empty'});
        } else {
          req.domain.emit('error', err);
        }
      } else {
