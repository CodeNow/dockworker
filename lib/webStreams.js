...
function buildHandler (remote) {
  ...
  function build () {
    ...
    var ts = mx.createStream('pty');
    var es = mx.createStream('ev');
    var term = spawn('bash', ['-c', 'echo start; sleep 1; echo progress; sleep 1; echo progress; sleep 1; echo stop']);
    term.stdout.on('data', function (d) {
      ts.write('\r' + d.toString());
    });
    term.stderr.on('data', function (d) {
      ts.write('\r' + d.toString());
    });
    ts.pipe(term.stdin);
    mx.pipe(remote);
...
var buildSock = shoe(buildHandler);
